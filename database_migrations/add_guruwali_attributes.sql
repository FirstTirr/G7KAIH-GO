-- Add guruwali attributes to user_profiles table
-- This allows teachers to have dual roles as guru wali

-- Add new columns
ALTER TABLE public.user_profiles 
ADD COLUMN is_guruwali boolean DEFAULT false,
ADD COLUMN is_guru_wali boolean DEFAULT false;

-- Update existing guruwali users (roleid = 6) to have is_guruwali = true
UPDATE public.user_profiles 
SET is_guruwali = true 
WHERE roleid = 6;
...
AS $$
  WITH current_user_profile AS (
    SELECT userid, kelas, is_guru_wali, roleid, is_guruwali
    FROM public.user_profiles 
    WHERE userid = auth.uid()
  )
  ...
  WHERE up_student.roleid = 5 -- student role
    AND (
      ...
      OR
      -- Class-based assignment (if current user is guru wali)
      (cup.is_guru_wali = true AND up_student.kelas = cup.kelas)
      OR
      -- If user is full guruwali, can supervise all students (optional - remove if too permissive)
      (cup.roleid = 6)
    );
$$;
